...
For information on obtaining JWT tokens, see the [Authentication guide](/promptql-apis/auth.mdx#obtaining-jwt-tokens).
...
1. **Use descriptive names**: Choose clear, descriptive names for your artifacts
2. **Leverage metadata**: Use metadata to categorize and tag your files
3. **Version control**: Update existing artifacts rather than creating duplicates
4. **Visibility settings**: Use appropriate visibility levels for security

### Performance

1. **Optimize file sizes**: Compress files when possible to reduce upload/download times
2. **Batch operations**: Group related file operations when possible
3. **Cache metadata**: Cache artifact metadata to reduce API calls

### Security

1. **Private by default**: Keep sensitive files private unless sharing is necessary
2. **Access control**: Regularly review artifact visibility settings
3. **Cleanup**: Remove unused artifacts to maintain security and organization
