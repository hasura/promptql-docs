...
curl -X POST "https://promptql.ddn.hasura.app/playground/threads/v2" \
  -H "Authorization: api-key ${PROMPTQL_API_KEY}" \
  -H "Content-Type: application/json"
  -d '{
      "user_message": "<your_message>",
      "ddn_headers": {
          "x-hasura-ddn-token": "${JWT_TOKEN}"
      },
...
Additionally, instead of passing the key-value pair of `x-hasura-ddn-token`, you can utilize `Authorization` as the key
with any authentication strategy the underlying DDN project supports.
