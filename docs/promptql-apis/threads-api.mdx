...
collection, evaluation, pinned threads management, and the new v2 endpoints with enhanced artifact handling and user
...
`https://promptql.ddn.hasura.app/playground`
...
`https://promptql.<FQDN>/playground`
...
```
Authorization: api-key <your-promptql-api-key>
Content-Type: application/json
```
...
- Thread feedback (`/threads/{thread_id}/feedback, /projects/{project_id}/feedbacks`)

:::info Getting JWT Tokens

For user-scoped operations, you need to obtain a JWT token using your Personal Access Token (PAT) or Project Service
Account Token. See the [Authentication Guide](/promptql-apis/auth.mdx#getting-jwt-tokens-for-user-scoped-operations) for
detailed instructions on how to get JWT tokens.
...
The v2 Threads API provides enhanced functionality with improved artifact handling, user confirmation workflows, and
better thread management capabilities.

### Start Thread v2

Create a new thread using the v2 API with enhanced features.

`POST /threads/v2/start`

#### Request Body

```json
{
  "user_message": "Tell me about my data",
  "ddn_headers": {
    "x-hasura-ddn-token": "such token"
  },
...
Continue an existing thread conversation with enhanced artifact support.

`POST /threads/v2/{thread_id}/continue`

#### Path Parameters

| Parameter   | Type   | Required | Description                    |
| ----------- | ------ | -------- | ------------------------------ |
| `thread_id` | string | Yes      | UUID of the thread to continue |
...
Retrieve a thread with enhanced metadata and artifact information.

`GET /threads/v2/{thread_id}`
...
```json
{
  ...
  "interactions": [
    {
      "interaction_id": "interaction-uuid",
      "user_message": "Analyze the sales data",
...
Retrieve a specific artifact from a thread by its identifier.

`GET /threads/v2/{thread_id}/artifacts/{artifact_identifier}`
...
| `artifact_identifier` | string | Yes      | Identifier of the artifact |
...
```json
{
  ...
  "metadata": {
    "rows": 150,
    "columns": 8,
    "created_by": "data_analysis_program"
  },
  "created_at": "2024-01-15T10:45:00Z"
}
```

### Get Thread Artifact Data

Download the actual data content of a thread artifact.

`GET /threads/v2/{thread_id}/artifacts/{artifact_identifier}/data`
...
Returns the artifact data in its native format (CSV, JSON, etc.) with appropriate content headers.

### Cancel Thread Interaction

Cancel an ongoing thread interaction.

`POST /threads/v2/{thread_id}/cancel`
...
```json
{
  "thread_id": "123e4567-e89b-12d3-a456-426614174000",
  "status": "cancelled",
  "message": "Thread interaction cancelled successfully"
}
```

### Submit User Confirmation

Submit a user confirmation response for workflows requiring approval.

`POST /threads/v2/{thread_id}/user-confirmation/{user_confirmation_id}`
...
| ---------------------- | ------ | -------- | -------------------------------- |
| `thread_id`            | string | Yes      | UUID of the thread               |
| `user_confirmation_id` | string | Yes      | UUID of the confirmation request |
...
### Get Thread Feedback List

Retrieve list of thread feedbacks for a project with date range filtering.

`GET /projects/{project_id}/feedbacks`

#### Path Parameters

| Parameter   | Type   | Required | Description                            |
| ----------- | ------ | -------- | -------------------------------------- |
| `project_id` | string | Yes      | UUID of the project to get feedbacks for |

#### Query Parameters

| Parameter         | Type     | Required | Description                                     |
| ----------------- | -------- | -------- | ----------------------------------------------- |
| `from_created_at` | datetime | No       | Filter feedbacks created after this timestamp   |
| `to_created_at`   | datetime | No       | Filter feedbacks created before this timestamp  |

#### Response

This is a map of thread IDs to list of feedbacks if present.
  ...
  "123e4567-e89b-12d3-a456-426614174000": [
    {
      ...
      "detail": "The response was very helpful and accurate",
      "created_at": "2024-01-15T10:30:00Z",
      "updated_at": "2024-10-14T16:45:00.789012Z"
    },
    {
      "thread_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
      "user_id": "11111111-2222-3333-4444-555555555555",
      ...
      "detail": "The response was not clear",
      "created_at": "2024-01-15T10:30:00Z",
      "updated_at": "2024-10-14T16:45:00.789012Z"
    }
  ]
