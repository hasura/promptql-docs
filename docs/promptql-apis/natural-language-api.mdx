...
Version 1 requests support both Authorization header (recommended) and the deprecated `promptql_api_key` field in the
request body. For new implementations, use the Authorization header with Bearer token format.
