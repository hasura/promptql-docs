...
We can easily do this by adding custom business logic using one of our
...
configuration files, including a file called `functions.ts` — this is where we'll add our logic so the application can
...
Before we create a new build, let's add a description to our command.
...
### Step 1. Crete a new cloud build
