...
```
arn:aws:bedrock:<AWS region>:<AWS account ID>:inference-profile/us.anthropic.claude-sonnet-4-5-20250929-v1:0
```
...
- `modelId` must be publisher-prefixed (`google/...` or `anthropic/...`)
- `googleLocation` is required; use a supported region (e.g., `global`, `us-central1`, `europe-west4`). Location
  references:
  - Google models: https://cloud.google.com/vertex-ai/generative-ai/docs/learn/locations#google_model_endpoint_locations
  - Partner models (e.g., Anthropic):
    https://cloud.google.com/vertex-ai/generative-ai/docs/learn/locations#genai-partner-models
- `googleProject` is required and must be a valid Google Cloud project ID.

Example configuration:

```yaml title="Example Vertex (Google)"
kind: PromptQlConfig
version: v2
definition:
  llm:
...
```yaml title="Example Vertex (Anthropic)"
kind: PromptQlConfig
version: v2
definition:
  llm:
...
:::warning Service account credentials
...
When using the `hasura` provider, the default model is `claude-sonnet-4-5-20250929` (Claude Sonnet 4.5). This is the recommended model for
PromptQL program generation.

:::tip Troubleshooting If you see “model not supported in streamGenerateContent”, ensure:
