...
```ddn title="From the local project directory, create a new cloud project:"
ddn project init
```
...
```ddn title="Use the --with-project flag to map the local directory to the parent cloud project, replacing the name with your parent project's name:"
...
```ddn title="Create a subgraph build on your DDN project:"
...
```ddn title="Once your local metadata is in its desired state, create a subgraph build on your DDN project:"
ddn subgraph build create
```
