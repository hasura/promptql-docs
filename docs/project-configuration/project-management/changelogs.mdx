...
:::info Edit from the console

As a project admin, you can also edit these entries from the console. Navigate to the `Builds` page and see all
available entries from the `Changelog` tab. Non-admin users who navigate here can only see published entries and are
unable to edit them.
