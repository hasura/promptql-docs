...
[How to Build with DDN section](/how-to-build-with-promptql/overview.mdx).
...
```bash title="Serve your build to enable PromptQL to access your data:"
ddn run docker-start
ddn console --local
```

Now when you ask PromptQL about users, it can automatically apply the `shoutName` function to transform user names into
uppercase, enhancing the analysis capabilities.

For example, when asking "Show me all user names and their shouted versions", PromptQL will use this relationship to
generate a query plan that fetches user data and applies the transformation:

```
User: "Show me all user names and their shouted versions"
```

PromptQL will generate a query plan that uses this relationship.

With relationships and custom commands, PromptQL can transform or enrich data automatically as part of its dynamic query
planning process, delivering more accurate and insightful responses to user questions.
