...
:::note Supported Connectors for PromptQL

Most Hasura DDN connectors are compatible with PromptQL, with SQL-based database connectors offering the best
integration since PromptQL communicates with the Hasura DDN engine using SQL.

Some limitations exist:

- Lambda and OpenAPI connectors that return nested data structures may have reduced reliability
- Connectors returning arbitrary JSON with primitive scalar values are not supported
- OpenAPI, command-based connectors may experience inconsistent results
