...
description:
  "Learn how to publish your own data connector to the PromptQL Connector Hub, empowering the entire community to use
  it!"
keywords:
  - hasura ddn
  - data sources
...
existing versions on the [Connector Hub](https://hasura.io/connectors), which is powered by
...
- Uploading connector packages to PromptQL Cloud
- Updating the Hub Registry database so connectors can be deployed on PromptQL Cloud
- Making a new connector/connector version available in the ecosystem
...
- **is_verified**: Set this to `false` as only connectors developed by PromptQL can be tagged as verified.
- **is_hosted_by_hasura**: Set this to `false` as connectors not developed by PromptQL cannot be hosted by PromptQL.
...
4. Wait for review and approval from the PromptQL team.

#### Post-approval process

Once your pull request is approved and merged:

- The new connector will be added to the Hub Registry.
...
2. Create a pull request targeting the `main` branch of the `ndc-hub` repository.
3. Wait for approval from a PromptQL team member.
...
- Update the PromptQL Hub Registry database.
...
- If validation is successful, it updates the connector information on the `staging` PromptQL environment.
