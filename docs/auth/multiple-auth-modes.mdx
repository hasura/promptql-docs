---
sidebar_position: 6
description: "Learn how to configure and use multiple authentication modes with PromptQL."
keywords:
...
header with the identifier of the desired mode. You can pass this value in via the
[PromptQL Playground](/auth/playground-auth.mdx).
...
- Keep in mind that the `X-Hasura-Auth-Mode` header may be included in standard HTTP request monitoring tools, just like
  other headers
- Consider using network controls (like IP allowlists) or the
  [Allowlist Plugin](https://hasura.io/docs/3.0/plugins/allowlist/) to manage which clients can use specific
