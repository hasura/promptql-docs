---
sidebar_position: 1
description: "Learn how the PiQL PromptQL chat client interface sets the auth headers for your requests."
keywords:
  - PiQL
  - Auth
  - Headers
...
You can check the auth which the PiQL client is using by clicking the Auth button on the left-hand side of the chat
dialogue.
...
The `x-hasura-ddn-token` header is automatically added by the PiQL client and should not be modified. If passing a
custom JWT in [JWT mode](/auth/jwt/jwt-mode.mdx) it should be set with another `x-hasura-*` header key.
