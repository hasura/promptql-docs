---
description:
  "Learn how to set up PromptQL DDN and PromptQL with Webhook mode to authenticate requests and apply access-control
  rules."
keywords:
  - hasura webhook mode
...
You can enable the engine to use an auth webhook in just a few steps.
...
The engine utilizes an [AuthConfig](/reference/metadata-reference/auth-config.mdx) object that allows you to define the
configuration for your authentication service. In a standard setup the `auth-config.hml` file can be found in your
`globals` directory.

:::tip PromptQL VS Code extension

You can use [PromptQL's VS Code extension](https://marketplace.visualstudio.com/items?itemName=HasuraHQ.hasura) to
...
For a POST request, the headers received by the engine on the query can be forwarded to the webhook in a JSON object in
the body of the request under a `headers` key.

For a GET request, the headers received by the engine on the query can be forwarded to the webhook as actual headers on
the request and the body will be empty.
...
engine on the query to the webhook.
...
[permissions](/reference/metadata-reference/permissions.mdx) in the engine.

You will, at least, need to set the `X-Hasura-Role` session variable to let the the engine know which role to use for
...
```json title="Example response from your webhook to the engine"
HTTP/1.1 200 OK
Content-Type: application/json
