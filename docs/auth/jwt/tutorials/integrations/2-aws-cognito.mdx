...
a JWT which you can pass in the header of your requests to the DDN engine. After setting up your
...
underlying data served by the DDN engine.
...
following code to modify the Cognito JWT and inject the custom DDN namespace claims:
...
This will add the required DDN namespace with the keys that the DDN engine expects when decoding a JWT. You can modify
...
`x-hasura-user-id` is simply an example. Any claim prefixed with `x-hasura-` is accessible to the DDN engine.
