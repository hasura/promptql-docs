...
description: "Learn how to connect AWS Cognito to your PromptQL DDN supergraph."
keywords:
  - hasura
  - hasura ddn
  - authentication
...
a JWT which you can pass in the header of your requests to the engine. After setting up your
...
underlying data served by the engine.
...
To add the custom claims that PromptQL requires, you will need to create an AWS Lambda function and set it as a trigger
...
following code to modify the Cognito JWT and inject the custom namespace claims:

```javascript
export const handler = async (event) => {
  // Here, you'll need to fetch the user's role from PromptQL DDN using an admin-level authenticated request
...
This will add the required namespace with the keys that the engine expects when decoding a JWT. You can modify the keys
to suit your application's [roles](/reference/metadata-reference/permissions.mdx#typepermissions).
...
`x-hasura-user-id` is simply an example. Any claim prefixed with `x-hasura-` is accessible to the engine.
