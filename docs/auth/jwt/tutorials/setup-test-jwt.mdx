...
By default, PromptQL uses your PromptQL cloud authentication token, also known as a personal access token (PAT), for
authentication. This is convenient for testing from the Playground, but should not be used in an application or shared
with others.
...
For more information about the claims the engine expects, check out [this page](/auth/jwt/jwt-configuration.mdx).
