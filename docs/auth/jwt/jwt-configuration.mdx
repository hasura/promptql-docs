...
the `x-hasura-role` header tells the DDN engine which role to use for the request, and if that is missing then the
...
This setup is designed so that there is one authoritative way to construct your JWT token for the DDN engine which can
cover a wide range of use cases.

## PromptQL DDN JWT Claim Description
...
Permissions and by specifying this list it lets the DDN engine know that this user can assume any of them.
...
- DDN engine only has access to headers and JWT claims which are prefixed with `x-hasura-`.
- DDN engine only has access to JWT claims in namespace defined in the `AuthConfig` object in metadata.
- All `x-hasura-*` values should be of type `String`, they will be converted to the right type automatically.

## PromptQL DDN JWT configuration options
...
The `claimsFormat` field indicates whether the PromptQL DDN-specific claims are a regular JSON object or a stringified
JSON. The following possible values are allowed: `Json`, `StringifiedJson`.
...
In this mode, the DDN engine expects an `Authorization` header with a `Bearer` token.
...
### PromptQL DDN JWT Config Examples
...
The `key` is the actual shared secret, which is used by the DDN engine and the external auth server.
...
the intended tenant for the JWT. Setting the `audience` field in the PromptQL DDN JWT configuration will make sure that
the `aud` claim from the JWT is also checked during verification. Not doing this check will allow JWTs issued for other
...
`connection_init` when the websocket is connected to DDN engine and is valid until the expiry of the JWT when in JWT
mode.
...
Currently, there is no workaround possible in DDN. Even if DDN strips the additional padding the signature verification
of the token would fail (as DDN had to tamper the token).
...
Auth0 with PromptQL DDN.

### Clerk

Clerk integrates with PromptQL DDN using JWTs.
...
PromptQL DDN with Clerk.

### Keycloak

By default, Keycloak uses the `RSA-OAEP` algorithm, which PromptQL DDN doesn't support. Remove the algorithm in the
