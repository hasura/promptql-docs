...
```ddn title="Login with the DDN CLI."
ddn auth login
```
...
```ddn title="Initialize supergraph."
ddn supergraph init .
```

```ddn title="Add a connector.  Here, we are using the -i flag for interactive mode."
ddn connector init -i
```

```ddn title="Introspect the data source.  Substitute <connector-name> with name you chose for your connector above."
ddn connector introspect <connector-name>
```

```ddn title="Add models, commands and relationships based on the output from previous command.  Substitute <connector-name> with name you chose for your connector above."
ddn model add <connector-name> "*"
ddn command add <connector-name> "*"
ddn relationship add <connector-name> "*"
```

```ddn title="Build the supergraph locally."
ddn supergraph build local
```

```ddn title="Run docker.  Ensure all containers start up successfully."
ddn run docker-start
```

```ddn title="Verify via local console."
ddn console --local
```
...
```ddn title="Create cloud project."
...
```ddn title="Create a build for your cloud project."
...
```ddn title="Record the ingress URL for your build version."
...
```ddn title="Record the ingress URL for your project's API."
...
```ddn title="Mark build as applied."
