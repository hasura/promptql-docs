...
| `maxTableArtifactSampleRows` | integer / null                                                                      | false    | Maximum number of rows to show to the LLM when sampling table artifacts                                                                  |
...
| `contextWindowLimit`         | integer / null                                                                      | false    | Maximum number of tokens to be used for the context window for threads.                                                                  |
...
| `enable_visualizations_v3` | boolean / null | false    | Enable the experimental visualizations v3 feature |
| `<customKey>`              |                | false    |                                                   |
...
| Key         | Value                                                  | Required | Description                                                                                         |
| ----------- | ------------------------------------------------------ | -------- | --------------------------------------------------------------------------------------------------- |
| `tables`    | [SchemaTableNames](#promptqlconfigv2-schematablenames) | false    | Schemas in the database. The key is the schema name. Values are lists of table names in the schema. |
| `functions` | [string]                                               | false    | Functions in the database. Values are the function names.                                           |

#### SchemaTableNames {#promptqlconfigv2-schematablenames}
...
| `modelId`                     | string                                                                | true     | The specific Gemini/Vertex model to use                          |
| `googleProject`               | string                                                                | true     | The GCP project ID used for Vertex AI                            |
| `googleLocation`              | string                                                                | true     | The Vertex AI location (e.g., us-central1)                       |
| `googleServiceAccountKeyJson` | [EnvironmentValue](#promptqlconfigv2-environmentvalue)                | true     | The Google service account key JSON to authenticate to Vertex AI |
| `safetySettings`              | [GeminiSafetySettings](#promptqlconfigv2-geminisafetysettings) / null | false    | Safety settings for Gemini-compatible models on Vertex           |
| `fallback`                    | [LlmConfig](#promptqlconfigv2-llmconfig) / null                       | false    | The fallback LLM to use if Vertex is unavailable                 |
