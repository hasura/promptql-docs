...
The JDBC connector supports two parameter syntaxes:

1. **Colon prefix syntax**: `:parameter` (used in configuration-based approach)
2. **Double curly braces syntax**: `{{parameter}}` (used in file-based approach)

Both syntaxes are translated to parameterized queries, which helps prevent SQL injection attacks.
