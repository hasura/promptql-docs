...
Query is specified in the [Native Operation syntax](/reference/connectors/databricks/native-operations/syntax.mdx) page.
...
1. Create a new directory structure under the connector configuration:
   ...
   ```sh
   mkdir -p my_subgraph/connector/databricks-connector/native_operations/queries/
   ```

2. Create a new SQL file `my_subgraph/connector/databricks-connector/native_operations/queries/artist_by_name_between.sql` with
...
3. Create a new entry in the connector configuration:
   ...
   the SQL file is expected to be present at `<ddn-project-root>/my_subgraph/connector/databricks-connector/native-operations/queries/artist_by_name_between.sql`

   ```sh
   ddn connector plugin --connector my_subgraph/connector/databricks-connector/connector.yaml -- \
...
4. If your data connector is running, run the following command to update your metadata to track the new native query:
   ...
   ```sh
   ddn connector-link update databricks_connector --add-all-resources
   ```
   ...
   ddn connector introspect databricks_connector

   # update your metadata to track the new native query
   ddn model add databricks_connector "artist_by_name_between"
   ```
...
```sh
ddn connector plugin --connector my_subgraph/connector/databricks_connector/connector.yaml -- \
