...
Native Queries allow you to run custom SQL queries on your Athena database. These allow you to run queries that are not supported by PromptQL by default.

## Structure

A Native Query is a single SQL statement that returns results and can take arguments. The SQL structure of a Native
Query is specified in the [Native Operation syntax](/reference/connectors/athena/native-operations/syntax.mdx) page.
...
1. Create a new directory structure under the connector configuration:
   ...
   ```sh
   mkdir -p my_subgraph/connector/athena-connector/native_operations/queries/
   ```

2. Create a new SQL file `my_subgraph/connector/athena-connector/native_operations/queries/artist_by_name_between.sql` with
...
3. Create a new entry in the connector configuration:
   ...
   the SQL file is expected to be present at `<ddn-project-root>/my_subgraph/connector/athena-connector/native-operations/queries/artist_by_name_between.sql`

   ```sh
   ddn connector plugin --connector my_subgraph/connector/athena-connector/connector.yaml -- \
...
4. If your data connector is running, run the following command to update your metadata to track the new native query:
   ...
   ```sh
   ddn connector-link update athena_connector --add-all-resources
   ```
   ...
   ddn connector introspect athena_connector

   # update your metadata to track the new native query
   ddn model add athena_connector "artist_by_name_between"
   ```
...
```sh
ddn connector plugin --connector my_subgraph/connector/athena_connector/connector.yaml -- \
