...
### Step 8. Optional: Add semantic information to your model
...
Add a description to the `users` table. Find the Model definition in the `users.hml` file in the `app/metadata/`
directory and add a description in the `definition` section.
...
### Step 12. Iterate on your PostgreSQL schema
...
#### Step 13.3. Describe your model

In the same way we described the `users` model, we can describe the `posts` model so that the LLM can better understand
the data and create appropriate query plans. Read more about providing
[semantic information here](/data-modeling/semantic-information.mdx#models).

#### Step 13.4. Create a new build

```sh title="Next, create a new build:"
ddn supergraph build local
```

#### Step 13.5. Restart your services

```sh title="Bring down the services by pressing CTRL+C and start them back up:"
ddn run docker-start
```

### Step 14. Chat about the posts
...
### Step 16. Add a description to the relationship

Again, it's important to provide semantic information to the LLM so that it can create appropriate query plans. Read
more about providing [semantic information here](/data-modeling/semantic-information.mdx#relationships).

### Step 17. Rebuild your project
...
### Step 18. Chat with your data using the relationship
